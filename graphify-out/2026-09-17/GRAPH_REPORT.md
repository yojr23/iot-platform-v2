# Graph Report - iot-platform-v2  (2026-09-17)

## Corpus Check
- 27 files · ~322,904 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3925 nodes · 7893 edges · 391 communities (169 shown, 113 thin omitted)
- Extraction: 96% EXTRACTED · 4% INFERRED · 0% AMBIGUOUS · INFERRED: 329 edges (avg confidence: 0.85)
- Token cost: 0 input · 0 output

## Community Hubs (Navigation)
- Auth & Security Tests
- Auth & Security Tests
- Sensor Readings Pipeline
- Sensor Readings Pipeline
- System Configuration
- Sensor Readings Pipeline
- Sensor Readings Pipeline
- Sensor Readings Pipeline
- Sensor Readings Pipeline
- Sensor Readings Pipeline
- Sensor Readings Pipeline
- Frontend Views
- Device Status & Monitoring
- Sensor Readings Pipeline
- Sensor Readings Pipeline
- Sensor Readings Pipeline
- Sensor Readings Pipeline
- Sensor Management
- Sensor Management
- Ingestion Service
- Sensor Management
- Sensor Management
- Ingestion Service
- Sensor Readings Pipeline
- Device Management
- Sensor Readings Pipeline
- Alert Management
- Sensor Management
- Ingestion Service
- Sensor Readings Pipeline
- Sensor Management
- Sensor Management
- Ingestion Service
- Sensor Management
- Dead Letter & Outbox
- Sensor Readings Pipeline
- Sensor Management
- Sensor Management
- Device Management
- Sensor Management
- Sensor Management
- Alert Management
- Sensor Management
- Device Management
- Sensor Management
- Sensor Readings Pipeline
- Sensor Readings Pipeline
- Sensor Management
- Sensor Management
- System Configuration
- Sensor Readings Pipeline
- Alert Management
- Ingestion Service
- Dead Letter & Outbox
- Sensor Management
- Device Management
- Frontend Realtime Layer
- Dead Letter & Outbox
- Sensor Readings Pipeline
- Community 59
- Sensor Readings Pipeline
- Sensor Readings Pipeline
- Alert Rules Engine
- Community 63
- Sensor Realtime & WebSocket
- Frontend Realtime Layer
- Ingestion Service
- Alert Management
- Community 68
- Frontend Realtime Layer
- User Roles & RBAC
- Device Management
- Sensor Readings Pipeline
- Ingestion Service
- Device Status & Monitoring
- Dead Letter & Outbox
- Sensor Readings Pipeline
- System Configuration
- Community 78
- Dead Letter & Outbox
- Community 80
- Device Management
- Device Status & Monitoring
- Sensor Readings Pipeline
- Sensor Readings Pipeline
- Dead Letter & Outbox
- Sensor Management
- Frontend Views
- Device Status & Monitoring
- Sensor Management
- Device Management
- Sensor Readings Pipeline
- Sensor Readings Pipeline
- Frontend Components
- Frontend Views
- Community 96
- Ingestion Service
- Community 98
- Gate Certification Tests
- Database & Migrations
- Sensor Management
- Alert Management
- Frontend Views
- Sensor Readings Pipeline
- Sensor Readings Pipeline
- Community 106
- Community 107
- Community 108
- Community 109
- Frontend Components
- Device Status & Monitoring
- Community 112
- Alert Management
- Alert Notifications & Email
- Ingestion Service
- Community 116
- Alert Management
- Device Status & Monitoring
- Alert Management
- Community 120
- Ingestion Service
- Community 122
- Device Status & Monitoring
- Community 126
- Frontend Views
- Device Status & Monitoring
- Sensor Readings Pipeline
- Sensor Readings Pipeline
- System Configuration
- Community 132
- Sensor Readings Pipeline
- Community 134
- Sensor Management
- Frontend Components
- Device Management
- Alert Management
- Sensor Management
- Sensor Management
- Community 143
- Database & Migrations
- Community 145
- Community 146
- Gate Certification Tests
- Community 148
- Sensor Readings Pipeline
- Sensor Readings Pipeline
- Frontend Realtime Layer
- Sensor Realtime & WebSocket
- Frontend State Management
- Frontend Views
- Database & Migrations
- Gate Certification Tests
- Gate Certification Tests
- Community 158
- Community 159
- Sensor Readings Pipeline
- Community 161
- Community 162
- Community 165
- Frontend Components
- Device Status & Monitoring
- Frontend Realtime Layer
- Frontend Views
- Database & Migrations
- Sensor Readings Pipeline
- Dashboard
- Alert Notifications & Email
- Ingestion Service
- Community 175
- Ingestion Service
- Community 177
- Frontend Views
- Frontend Views
- Frontend Views
- Frontend Views
- Database & Migrations
- Database & Migrations
- Sensor Management
- Database & Migrations
- System Configuration
- Community 187
- Community 188
- Ingestion Service
- Community 190
- Community 191
- Database & Migrations
- Frontend Components
- Alert Management
- Sensor Readings Pipeline
- Community 196
- Sensor Management
- Gate Certification Tests
- Alert Management
- Ingestion Service
- Database & Migrations
- Community 203
- Community 204
- System Configuration
- Community 237
- Community 238
- Community 239
- Sensor Readings Pipeline
- Sensor Management
- Sensor Readings Pipeline
- Community 243
- Frontend Components
- Sensor Management
- Device Status & Monitoring
- Community 247
- Frontend Views
- Gate Certification Tests
- Public API Surface
- Community 251
- Community 252
- Dashboard
- Community 254
- Community 255
- Community 256
- Alert Management
- Community 259
- Gate Certification Tests
- Gate Certification Tests
- Community 264
- Ingestion Service
- Community 266
- Dashboard
- Community 268
- Sensor Management
- Community 270
- Community 271
- Community 272
- Community 273
- Community 274
- Community 275
- Community 276
- Community 329
- Community 330
- Community 331
- Ingestion Service
- Community 333
- Community 334
- Community 335
- Community 336
- Gate Certification Tests
- Gate Certification Tests
- Gate Certification Tests
- Gate Certification Tests
- Community 341
- Community 342
- Community 343
- Community 344
- Gate Certification Tests
- Community 346
- Community 347
- Community 348
- Ingestion Service
- Gate Certification Tests
- Ingestion Service
- Ingestion Service
- Ingestion Service
- Ingestion Service
- Ingestion Service
- Ingestion Service
- Ingestion Service
- Community 373
- Community 374
- Community 375
- Community 376
- Community 377
- Community 378
- Community 379
- Community 380
- Community 381
- Community 382
- Community 383
- Community 384
- Alert Management
- Database & Migrations
- Database & Migrations
- Database & Migrations
- Community 389
- Gate Certification Tests

## God Nodes (most connected - your core abstractions)
1. `User` - 281 edges
2. `Sensor` - 238 edges
3. `Device` - 208 edges
4. `TestCase` - 193 edges
5. `SensorReading` - 104 edges
6. `Alert` - 103 edges
7. `AlertRule` - 82 edges
8. `SensorType` - 75 edges
9. `Controller` - 69 edges
10. `SystemSetting` - 66 edges

## Surprising Connections (you probably didn't know these)
- `chart.js` --references--> `Functional Inventory: Frontend SPA Table`  [INFERRED]
  back/package.json → memory/11-functional-inventory.md
- `useAlertsRealtime` --semantically_similar_to--> `channelRegistry`  [INFERRED] [semantically similar]
  docs/implementation/ownership.md → front_rebuild_plan/SINOA_Agentic_Dashboard_Implementation_Plan_v2.1_PUBLIC_REALTIME.md
- `Responsive Audit Matrix (33 Rows)` --semantically_similar_to--> `44px Minimum Touch Target Remediation`  [INFERRED] [semantically similar]
  task-10-report.md → task-10-remediation-report.md
- `Debezium CDC Pipeline` --conceptually_related_to--> `raw:consume Command`  [INFERRED]
  scripts/gate10/README.md → ingestion_service/README.md
- `SINOA Lab Blue Frontend Handoff` --references--> `LabShell`  [INFERRED]
  front/docs/design/LAB_BLUE_HANDOFF.md → docs/implementation/lab-blue-resource-views-2026-09-10.md

## Import Cycles
- None detected.

## Hyperedges (group relationships)
- **CDC Pipeline: MySQL Outbox → Debezium → Redis → Application Stream → Raw Consumer** — scripts_gate10_readme_mysql_outbox, scripts_gate10_readme_debezium_cdc, scripts_gate10_readme_fault_scenarios, ingestion_service_readme_raw_process_v1, ingestion_service_readme_raw_consume [EXTRACTED 0.95]
- **Authenticated Graph Catalog: Endpoint → Controller → Zones Mapping → Frontend Client** — task_8_report_dashboard_graph_catalog, task_8_report_dashboard_graph_catalog_controller, task_8_report_rule_to_graph_zones, task_8_report_getauthenticatedgraphcatalog, task_8_report_sanctum_auth [EXTRACTED 0.95]
- **Graph Series Window Validation: Controllers → Service → HTTP 422** — task_9_report_publicgraph_controller, task_9_report_sensor_api_controller, task_9_report_publicgraphseries_service, task_9_report_window_validation [EXTRACTED 0.95]
- **Fase 6B Blade Cleanup Gating Decision** — memory_14_blade_cleanup_readiness_verdict_nogo, memory_13_manual_validation_checklist_checklist, memory_12_stable_blade_comparison_missing_features, memory_06_pending_risks_operational_risks, memory_08_refactor_checklist_fase_6b [EXTRACTED 1.00]
- **Sensor reading to alert notification pipeline** — code_back_app_http_controllers_api_sensorapicontroller, code_back_app_observers_sensorreadingobserver, code_back_app_models_sensorreading, code_back_app_services_alerts_alertservice, code_back_app_observers_alertobserver, code_back_app_services_notifications_notificationservice, code_back_app_events_newalerttriggered, code_back_app_mail_dangeralertmail [EXTRACTED 1.00]
- **ICONTEC/ISO normative compliance document set** — docs_icontec_compliance, docs_matriz_trazabilidad_icontec_iso, docs_evidencia_cumplimiento_codigo_estructura, docs_procedimiento_auditoria_normativa, docs_referencias_icontec, docs_plantilla_trabajo_icontec, analisis_proyecto [EXTRACTED 1.00]
- **Ingestion Data Pipeline** — docs_superpowers_plans_2026_09_08_gate_s5r_closure_md_sensor_reading_service, docs_superpowers_plans_2026_09_08_gate_s5r_closure_md_raw_reading_normalizer, docs_superpowers_plans_2026_09_08_gate_s5r_closure_md_domain_event_broadcast_consumer, docs_superpowers_plans_2026_09_08_gate_s5r_closure_md_new_sensor_reading, docs_implementation_ownership_md_raw_sensor_event_publisher [EXTRACTED 1.00]
- **Docker Stack Core Event Processing Verified (M9–M12)** — consumer_topology_concept, consumer_crash_recovery_concept, m11_vertical_slice_concept, m9_upgrade_path_concept, docs_gate10_mac_verification_evidence_2026_09_16_md [EXTRACTED 1.00]
- **All 6 CI Jobs Verified GREEN on Mac** — backend_476_pass, responsive_e2e_35_35_pass, docs_gate10_mac_verification_evidence_2026_09_16_md, front_audit_e2e_task10_matrix_txt [EXTRACTED 1.00]
- **Front/Back Migration Phase Roadmap (Fase 0 to Fase 8A)** — memory_05_migration_log_fase_0, memory_05_migration_log_fase_1, memory_05_migration_log_fase_2, memory_05_migration_log_fase_3, memory_05_migration_log_fase_4, memory_05_migration_log_fase_5, memory_05_migration_log_fase_6, memory_05_migration_log_fase_7, memory_05_migration_log_fase_7b, memory_05_migration_log_fase_8a [EXTRACTED 1.00]
- **Public Graph Visibility Boundary** — front_rebuild_plan_public_graph_visibility_agentic_execution_plan_md_public_graph_visibility_service, front_rebuild_plan_public_graph_visibility_agentic_execution_plan_md_public_graph_controller, front_rebuild_plan_public_graph_visibility_agentic_execution_plan_md_public_graph_series_service, docs_superpowers_plans_2026_09_08_gate_s5r_closure_md_domain_event_broadcast_consumer [EXTRACTED 1.00]
- **Role Access Control Boundary** — docs_superpowers_plans_2026_09_11_role_access_control_md_role_access_control, docs_superpowers_specs_2026_09_11_role_access_control_design_md_role_access_design, docs_superpowers_plans_2026_09_11_role_access_control_md_can_manage_dashboard, docs_security_security_hardening_plan_md_resource_access_service [INFERRED 0.85]
- **SPA Auth & Security Boundary Design** — memory_01_architecture_decisions_adr_005_env_separation, memory_01_architecture_decisions_adr_006_api_only_communication, memory_01_architecture_decisions_adr_009_sanctum_bearer_auth, memory_15_security_review_sec_003_localstorage_bearer_token [INFERRED 0.85]
- **CDC Outbox-Binlog-Redis Pipeline** — plan_md.Transactional_Outbox, plan_md.debezium, plan_md.redis_streams, plan_md_cdc_pipeline, plan_md.cdc_outbox_stream_consumer, plan_md.outbox_cdc_consumer [EXTRACTED 0.95]
- **Sensor Authorization Four-Cell Matrix** — plan_md.sec_rt_002, plan_md.sensor_view, plan_md.sensor_reading_view, plan_md.resource_access_service, docs_mac_certification_evidence_md_authorization_consistency_fix, docs_mac_certification_evidence_md_resource_access_service [EXTRACTED 0.95]
- **Gate 10 CI Enforcement Pipeline** — github_workflows_gate10_quality_yml_gate10_quality, github_workflows_gate10_quality_yml_backend_job, github_workflows_gate10_quality_yml_frontend_job, github_workflows_gate10_quality_yml_frontend_responsive_e2e_mocked_job, github_workflows_gate10_quality_yml_ingestion_job, github_workflows_gate10_quality_yml_dependency_security_job, github_workflows_gate10_quality_yml_architecture_job, github_workflows_gate10_quality_yml_no_polling_static_gate [EXTRACTED 0.95]

## Communities (391 total, 113 thin omitted)

### Community 0 - "Auth & Security Tests"
Cohesion: 0.03
Nodes (15): User, AlertPolicy, DevicePolicy, AdminAccessTest, AuthSecurityTest, BroadcastAuthorizationTest, EmailSecretStorageTest, ConfigGeneralUpdateTest (+7 more)

### Community 1 - "Auth & Security Tests"
Cohesion: 0.03
Nodes (30): AlertRuleCascadeDeleteTest, RateLimitSecurityTest, ApiAuthTokenTest, AuthSecurityTest, ConfigSystemInfoTest, DataIntegrityDuplicationTest, DeviceCreateTest, DeviceStatusSnapshotTest (+22 more)

### Community 2 - "Sensor Readings Pipeline"
Cohesion: 0.05
Nodes (27): PurgeFutureSensorReadings, EvaluateSensorReadingAlerts, QueueSmokeJob, SendDangerAlertEmailJob, SensorReading, SystemSetting, SensorReadingObserver, EventServiceProvider (+19 more)

### Community 3 - "Sensor Readings Pipeline"
Cohesion: 0.04
Nodes (13): Device, DeviceStatusLog, DataLeakageSentinelTest, DeviceAuthorizationTest, ResourcePayloadTest, ApiRoutingRegressionTest, DeviceApiKeyVisibilityTest, DeviceApiStatusUpdateTest (+5 more)

### Community 4 - "System Configuration"
Cohesion: 0.06
Nodes (32): current_sha(), main(), parse_args(), Any, Namespace, Path, Write a short human index derived exclusively from the evidence document., write_json() (+24 more)

### Community 5 - "Sensor Readings Pipeline"
Cohesion: 0.04
Nodes (10): Sensor, RealtimeAuthorizationRegressionTest, SensorAuthorizationTest, SensorExportSecurityTest, BroadcastChannelAuthorizationTest, Gate6PublicSurfaceTest, SensorApiControllerTest, SensorGraphZonesTest (+2 more)

### Community 6 - "Sensor Readings Pipeline"
Cohesion: 0.05
Nodes (13): AlertRule, SensorType, AlertRuleNameTest, AlertRuleValidationTest, DashboardGraphCatalogControllerTest, SecuritySqlInjectionTest, AlertUniqueConstraintTest, RuleToGraphZonesTest (+5 more)

### Community 7 - "Sensor Readings Pipeline"
Cohesion: 0.07
Nodes (9): DashboardController, LabController, DeviceController, LabController, Throwable, Throwable, Throwable, Lab (+1 more)

### Community 8 - "Sensor Readings Pipeline"
Cohesion: 0.05
Nodes (15): AlertFactory, AlertRuleFactory, DeviceFactory, DeviceStatusLogFactory, DeviceTypeFactory, DomainEventOutboxFactory, LabFactory, RawEventOutboxFactory (+7 more)

### Community 9 - "Sensor Readings Pipeline"
Cohesion: 0.08
Nodes (8): Alert, DomainEventOutbox, AlertResolveTransitionTest, AlertTransportAuthorizationTest, AlertTriggerTransitionTest, AlertAuthorizationTest, AuthorizationMatrixTest, Illuminate\Support\Facades\Event

### Community 10 - "Sensor Readings Pipeline"
Cohesion: 0.06
Nodes (46): Lab Blue Resource Views, LabShell, AlertService, BaseModal, Echo JS, Reuse and Ownership Freeze Matrix, Polling Inventory, RawSensorEventPublisher (+38 more)

### Community 11 - "Frontend Views"
Cohesion: 0.05
Nodes (33): apps, flush(), mount(), catalogConfigs, config, deletingId, editingId, error (+25 more)

### Community 12 - "Device Status & Monitoring"
Cohesion: 0.05
Nodes (40): applyDevicesPage(), authStore, closeForm(), defaultDeviceForm(), deleteSelectedDevice(), deviceForm, devicePayload(), devices (+32 more)

### Community 13 - "Sensor Readings Pipeline"
Cohesion: 0.08
Nodes (38): ANALISIS_PROYECTO.md (analisis tecnico y de cumplimiento), Architecture Audit and Implementation Plan, PurgeFutureSensorReadings command, DeviceStatusUpdated event, NewAlertTriggered event, NewSensorReading event, SensorApiController, EmailConfigController (+30 more)

### Community 14 - "Sensor Readings Pipeline"
Cohesion: 0.09
Nodes (8): AlertFeedController, PublicGraphController, AlertService, SensorReadingProjectionService, DateTimeInterface, PublicGraphSeriesService, RuleToGraphZones, Illuminate\Support\Collection

### Community 15 - "Sensor Readings Pipeline"
Cohesion: 0.08
Nodes (6): AuthApiController, UserRoleController, EmailConfigController, SensorController, AuditService, Illuminate\Http\Request

### Community 16 - "Sensor Readings Pipeline"
Cohesion: 0.09
Nodes (16): UpdateDeviceLastCommunication, AlertObserver, Illuminate\Auth\Events\PasswordReset, Illuminate\Auth\Events\Registered, Illuminate\Auth\Events\Verified, Illuminate\Database\QueryException, Illuminate\Foundation\Application, Illuminate\Foundation\Configuration\Exceptions (+8 more)

### Community 17 - "Sensor Management"
Cohesion: 0.06
Nodes (5): app, authStore, pinia, router, useAuthStore

### Community 18 - "Sensor Management"
Cohesion: 0.06
Nodes (24): alerts, {
    auth,
    canManageDashboard,
    widgets,
    selectedId,
    selected,
    editing,
    saveState,
    message,
    removed,
    catalog,
    sensor,
    points,
    viewModel,
    widgetViewModel,
    widgetError,
    activeData,
    history,
    dirty,
    add,
    select,
    changeDevice,
    changeSensor,
    changeRange,
    remove,
    undo,
    move,
    save,
    load,
}, critical, dialog, dragged, expanded, expandedWidgets, isMobile (+16 more)

### Community 19 - "Ingestion Service"
Cohesion: 0.09
Nodes (13): InternalMetricsController, MetricsController, MetricsController, EnsureIngestionToken, EnsureUserHasPermission, EnsureUserIsAdmin, TrackApiPerformance, ApiMetricsService (+5 more)

### Community 20 - "Sensor Management"
Cohesion: 0.06
Nodes (13): App\Http\Controllers\Api\AuthApiController, App\Http\Controllers\Api\DashboardGraphCatalogController, App\Http\Controllers\Api\HealthController, App\Http\Controllers\Api\IngestionController, App\Http\Controllers\Api\InternalMetricsController, App\Http\Controllers\Api\PublicGraphController, AlertRuleController, RoleController (+5 more)

### Community 21 - "Sensor Management"
Cohesion: 0.07
Nodes (30): authStore, closeForm(), defaultSensorForm(), deleteSelectedSensor(), devices, editingSensorId, error, filteredSensors (+22 more)

### Community 22 - "Ingestion Service"
Cohesion: 0.11
Nodes (21): DurableEventSpool, PendingEvent, Mark event as failed. Returns True if event is now dead-lettered (exceeded max…, Move an exhausted event to the dead_letter table for later inspection., Return all dead-lettered events for inspection., The durable, local owner of MQTT events awaiting backend delivery., Return the spool's current time source for coordinated components., event() (+13 more)

### Community 23 - "Sensor Readings Pipeline"
Cohesion: 0.11
Nodes (9): AuditLog, DashboardPreference, Permission, RawEventOutbox, Collection, Illuminate\Database\Eloquent\Factories\HasFactory, Illuminate\Database\Eloquent\Model, Illuminate\Database\Eloquent\Relations\BelongsToMany (+1 more)

### Community 24 - "Device Management"
Cohesion: 0.08
Nodes (18): buildAuthTransitionEvidence(), isAuthTransitionPass(), consoleErrors, diagnoseOverflow(), __dirname, failedRequests, interact, mode (+10 more)

### Community 25 - "Sensor Readings Pipeline"
Cohesion: 0.12
Nodes (6): App\Services\Ingestion\SensorReadingProjectionService, App\Services\Ingestion\SensorReadingService, App\Services\Monitoring\PublicGraphSeriesService, App\Services\Monitoring\RuleToGraphZones, App\Services\SensorMappingService, SensorApiController

### Community 26 - "Alert Management"
Cohesion: 0.10
Nodes (5): AlertController, AlertController, AlertResource, AlertLifecycleService, DomainEventRecorder

### Community 27 - "Sensor Management"
Cohesion: 0.11
Nodes (6): ConfigController, DashboardGraphCatalogController, EmailConfigController, HealthController, DashboardPreferenceController, Illuminate\Http\JsonResponse

### Community 28 - "Ingestion Service"
Cohesion: 0.14
Nodes (3): PublicGraphControllerTest, ReadingTimeSemanticsTest, CarbonImmutable

### Community 29 - "Sensor Readings Pipeline"
Cohesion: 0.08
Nodes (22): accessibleSummary, chartContainer, chartData, chartOptions, hasData, props, renderChart(), sampleViewModel (+14 more)

### Community 30 - "Sensor Management"
Cohesion: 0.09
Nodes (14): ProfileController, ConfirmPasswordController, ForgotPasswordController, LoginController, VerificationController, Controller, HomeController, Illuminate\Foundation\Auth\Access\AuthorizesRequests (+6 more)

### Community 31 - "Sensor Management"
Cohesion: 0.12
Nodes (20): build_qc_checksum(), build_sensor_payload(), device_sender_worker(), get_api_key(), get_location(), get_node_id(), get_sensor_key(), get_sensors() (+12 more)

### Community 32 - "Ingestion Service"
Cohesion: 0.14
Nodes (17): Exception, BackendClient, BackendClientError, Any, RuntimeError, Raised when backend ingestion endpoint cannot be reached or rejects payload., DeliveryWorker, retry_delay() (+9 more)

### Community 33 - "Sensor Management"
Cohesion: 0.12
Nodes (8): AlertRuleController, StoreAlertRuleRequest, UpdateAlertRuleRequest, AlertRuleResource, DeviceResource, DeviceStatusSnapshotResource, SensorResource, Illuminate\Http\Resources\Json\JsonResource

### Community 34 - "Dead Letter & Outbox"
Cohesion: 0.20
Nodes (3): DomainEventBroadcastConsumerTest, Connection, RedisManager

### Community 35 - "Sensor Readings Pipeline"
Cohesion: 0.19
Nodes (3): RawSensorEvent, RawReadingNormalizerTest, RawSensorEventIdempotencyTest

### Community 36 - "Sensor Management"
Cohesion: 0.11
Nodes (10): AlertRuleSeeder, AlertSeeder, DeviceTypeSeeder, RolePermissionSeeder, SensorTypeSeeder, SystemSettingsSeeder, UserSeeder, UserSeederSecurityTest (+2 more)

### Community 37 - "Sensor Management"
Cohesion: 0.15
Nodes (4): IngestionController, IngestionApiTest, Illuminate\Database\UniqueConstraintViolationException, PDOException

### Community 38 - "Device Management"
Cohesion: 0.13
Nodes (4): DeviceTypeController, ConfigController, DeviceTypeController, DeviceType

### Community 39 - "Sensor Management"
Cohesion: 0.12
Nodes (6): StoreRawIngestionEventRequest, UpdateAlertConfigRequest, UpdateEmailConfigRequest, UpdateGeneralConfigRequest, Illuminate\Foundation\Http\FormRequest, Illuminate\Validation\Validator

### Community 40 - "Sensor Management"
Cohesion: 0.16
Nodes (19): ranges, api, apps, mount(), useLabWorkspace(), add(), changeDevice(), changeRange() (+11 more)

### Community 41 - "Alert Management"
Cohesion: 0.15
Nodes (23): ALERTS_CHANNEL, ALERTS_EVENT, ALERTS_EVENT_CLASS, ALERTS_RESOLVED_EVENT, ALERTS_RESOLVED_EVENT_CLASS, bufferDuringRecovery(), CONNECTION_ERROR_MESSAGES, error (+15 more)

### Community 42 - "Sensor Management"
Cohesion: 0.09
Nodes (18): authStore, device, deviceStatuses, effectiveDevice, error, loading, oneTimeApiKey, props (+10 more)

### Community 43 - "Device Management"
Cohesion: 0.09
Nodes (21): apiMetrics, apiRequestsChart, barOptions, comparisonChart, devicesChart, doughnutOptions, error, gaugeOptions (+13 more)

### Community 44 - "Sensor Management"
Cohesion: 0.13
Nodes (8): ConsumeCdcOutboxes, Closure, DomainEventPublisher, RawSensorEventPublisher, CountingDomainPublisher, CountingRawPublisher, RedisManager, EventPublisherXaddShapeTest

### Community 45 - "Sensor Readings Pipeline"
Cohesion: 0.17
Nodes (6): DeviceSensorMapping, self, SensorMappingService, Carbon\Carbon, DateTimeInterface, Illuminate\Support\Str

### Community 46 - "Sensor Readings Pipeline"
Cohesion: 0.10
Nodes (21): authStore, canExportTelemetry, canViewTelemetry, error, exporting, exportReadings(), filterActive, filteredReadings (+13 more)

### Community 47 - "Sensor Management"
Cohesion: 0.13
Nodes (22): Admin Routes RBAC Correction (admin-only list narrowed), Backend PHP Suite 476/476 PASS (1826 assertions), CdcOutboxStreamConsumer callable→Closure Fix, Consumer Crash Recovery (no-loss, no-manual-intervention), Consumer Topology (outbox-cdc-consumer, raw-consumer, domain-event-consumer), Device Isolation via X-Device-Key, Gate 10 Mac Verification Evidence Ledger, Task 10 / Gate 9 Mocked Responsive Chromium Matrix (+14 more)

### Community 48 - "Sensor Management"
Cohesion: 0.15
Nodes (9): App\Http\Controllers\Controller, App\Http\Resources\DeviceResource, App\Http\Resources\DeviceStatusSnapshotResource, App\Http\Resources\SensorResource, App\Models\Device, App\Models\DomainEventOutbox, DeviceApiController, Device (+1 more)

### Community 49 - "System Configuration"
Cohesion: 0.10
Nodes (13): MailMessage, ResetPasswordNotification, MailMessage, VerifyEmailNotification, Illuminate\Auth\Access\AuthorizationException, Illuminate\Auth\Notifications\ResetPassword, Illuminate\Auth\Notifications\VerifyEmail, Illuminate\Contracts\Auth\MustVerifyEmail (+5 more)

### Community 50 - "Sensor Readings Pipeline"
Cohesion: 0.17
Nodes (19): BaseModel, build_raw_event(), derive_source_event_id(), _identity_part(), _iso_now(), Any, Build a retry-stable identity from device-domain fields, never MQTT packet IDs., _source_identity() (+11 more)

### Community 51 - "Alert Management"
Cohesion: 0.11
Nodes (19): alerts, alertsStore, error, filter, handleResolve(), handleResolveAll(), load(), loading (+11 more)

### Community 52 - "Ingestion Service"
Cohesion: 0.13
Nodes (5): DebeziumChange, self, UsesRawRedisCommands, Illuminate\Redis\Connections\Connection, RuntimeException

### Community 54 - "Sensor Management"
Cohesion: 0.10
Nodes (4): UpdateAlertRulesTable, UpdateAlertRulesSeverity, up(), Illuminate\Database\Migrations\Migration

### Community 55 - "Device Management"
Cohesion: 0.12
Nodes (20): closeModal(), deleteRule(), deletingId, error, formError, load(), loading, loadMetadata() (+12 more)

### Community 56 - "Frontend Realtime Layer"
Cohesion: 0.13
Nodes (13): __dirname, fakeEchoPlugin(), main(), record(), results, root, __dirname, fakeEchoPlugin() (+5 more)

### Community 58 - "Sensor Readings Pipeline"
Cohesion: 0.18
Nodes (15): alert(), countFor(), device(), deviceSensor(), installAuth(), json(), mockApi(), readings() (+7 more)

### Community 59 - "Community 59"
Cohesion: 0.19
Nodes (14): buildMatrixSummaryRecord(), isPassingMatrixRecord(), parseMatrixRow(), parsedRun, hasInteractionFailure(), isCleanResult(), isPageCorrect(), isPassingAuditResult() (+6 more)

### Community 60 - "Sensor Readings Pipeline"
Cohesion: 0.17
Nodes (7): App\Models\DeviceSensorMapping, App\Models\SensorReading, DeviceUpdateAtomicityTest, SensorProvisioningAtomicityTest, User, Database\Seeders\RolePermissionSeeder, Tests\TestCase

### Community 61 - "Sensor Readings Pipeline"
Cohesion: 0.22
Nodes (7): App\Models\Permission, App\Models\Role, App\Models\Sensor, App\Models\User, SensorPolicy, ResourceAccessService, Illuminate\Support\Facades\Broadcast

### Community 62 - "Alert Rules Engine"
Cohesion: 0.14
Nodes (7): RegisterController, ResetPasswordController, AlertRuleValidationService, Illuminate\Foundation\Auth\RegistersUsers, Illuminate\Foundation\Auth\ResetsPasswords, Illuminate\Support\Facades\Validator, Illuminate\Validation\Rules\Password

### Community 63 - "Community 63"
Cohesion: 0.11
Nodes (18): bootstrap, bootstrap, @fontsource/inter, @fontsource/jetbrains-mono, dependencies, axios, bootstrap, @fontsource/inter (+10 more)

### Community 64 - "Sensor Realtime & WebSocket"
Cohesion: 0.11
Nodes (15): activeAlertsCard, alertsRealtime, alertsStore, alertsView, alertToast, appLayout, echo, envExample (+7 more)

### Community 65 - "Frontend Realtime Layer"
Cohesion: 0.18
Nodes (15): booleanEnv(), createEcho(), disconnectEcho(), getEcho(), getEchoConfig(), handleAuthChange(), ADR-0002, notify() (+7 more)

### Community 66 - "Ingestion Service"
Cohesion: 0.23
Nodes (12): configure_logging(), main(), parse_args(), Any, Namespace, run_mqtt(), run_simulation(), _sample_payload() (+4 more)

### Community 67 - "Alert Management"
Cohesion: 0.13
Nodes (13): icons, paths, props, alerts, auth, links, logout(), open (+5 more)

### Community 68 - "Community 68"
Cohesion: 0.12
Nodes (17): devDependencies, axios, concurrently, laravel-vite-plugin, @popperjs/core, sass, tailwindcss, @tailwindcss/vite (+9 more)

### Community 69 - "Frontend Realtime Layer"
Cohesion: 0.12
Nodes (17): scripts, audit:baseline, audit:events, audit:gate9, audit:network, audit:network:live, audit:no-polling:source, build (+9 more)

### Community 71 - "Device Management"
Cohesion: 0.14
Nodes (13): auth, error, graphDevices, loading, boardDevices, flush(), getActiveAlerts, getAuthenticatedGraphCatalog (+5 more)

### Community 72 - "Sensor Readings Pipeline"
Cohesion: 0.13
Nodes (17): Dead Letter Queue (DLQ), Exponential Backoff Delivery, Idempotency Ledger via status Column, MQTT Ingestion, POST /api/ingestion/events, QC Validation Gate, raw:consume Command, raw-process-v1 Redis Consumer Group (+9 more)

### Community 74 - "Device Status & Monitoring"
Cohesion: 0.22
Nodes (15): onConnectionStateChange(), DEVICE_STATUS_CHANNEL, DEVICE_STATUS_EVENT, DEVICE_STATUS_EVENT_CLASS, error, eventPayload(), fetchStatusSnapshot(), isConnected (+7 more)

### Community 75 - "Dead Letter & Outbox"
Cohesion: 0.17
Nodes (6): ConsumeDomainEvents, ConsumeRawEvents, InspectDeadLetters, InspectReadingTimeSemantics, ReplayDeadLetter, Illuminate\Console\Command

### Community 77 - "System Configuration"
Cohesion: 0.13
Nodes (15): scripts, post-autoload-dump, post-create-project-cmd, post-root-package-install, post-update-cmd, test, Illuminate\\Foundation\\ComposerScripts::postAutoloadDump, @php artisan config:clear --ansi (+7 more)

### Community 79 - "Dead Letter & Outbox"
Cohesion: 0.34
Nodes (3): Connection, RedisManager, RawStreamConsumerTest

### Community 80 - "Community 80"
Cohesion: 0.13
Nodes (15): devDependencies, jsdom, @playwright/test, sass, vite, @vitejs/plugin-vue, vitest, ws (+7 more)

### Community 81 - "Device Management"
Cohesion: 0.15
Nodes (12): alertsStore, currentAlert, currentDevice, currentMessage, currentSeverity, currentTime, currentTitle, currentValue (+4 more)

### Community 82 - "Device Status & Monitoring"
Cohesion: 0.14
Nodes (5): props, severityClass, apps, authStore, mountedApps

### Community 83 - "Sensor Readings Pipeline"
Cohesion: 0.31
Nodes (8): DeviceCommunicationReceived, Illuminate\Broadcasting\Channel, Illuminate\Broadcasting\InteractsWithSockets, Illuminate\Broadcasting\PresenceChannel, Illuminate\Broadcasting\PrivateChannel, Illuminate\Contracts\Broadcasting\ShouldBroadcastNow, Illuminate\Foundation\Events\Dispatchable, Illuminate\Queue\SerializesModels

### Community 84 - "Sensor Readings Pipeline"
Cohesion: 0.20
Nodes (3): ReadingProjection, ReadingProvenanceService, Illuminate\Database\Eloquent\Relations\BelongsTo

### Community 85 - "Dead Letter & Outbox"
Cohesion: 0.26
Nodes (4): DeadLetterCommandsTest, Connection, RedisManager, Illuminate\Redis\RedisManager

### Community 86 - "Sensor Management"
Cohesion: 0.14
Nodes (14): iot.dead-letter-events Stream, iot.raw-events Stream, Poison-to-DLQ Live Certification (Phase H4), Architecture CI Job (File-Existence Assertions), Transactional Outbox Pattern, ADR-1 — Durable Publication, CdcOutboxStreamConsumer, ConsumeCdcOutboxes Artisan Command (+6 more)

### Community 87 - "Frontend Views"
Cohesion: 0.15
Nodes (10): email, error, errors, loading, payload(), save(), saving, success (+2 more)

### Community 88 - "Device Status & Monitoring"
Cohesion: 0.24
Nodes (5): App\Services\Ingestion\DomainEventRecorder, DeviceService, Device, DeviceServiceTest, DomainEventRecorder

### Community 91 - "Sensor Readings Pipeline"
Cohesion: 0.24
Nodes (12): ADR-0002, onResync(), normalizeReading(), RECOVERY_WINDOW_MS, resolveSensorId(), SENSOR_EVENT, SENSOR_EVENT_CLASS, useSensorRealtime() (+4 more)

### Community 92 - "Sensor Readings Pipeline"
Cohesion: 0.15
Nodes (13): Laravel API (back), MySQL Mapping Concurrency Proof (Phase D), Redis, SensorMappingService, Transactional Integrity Fix (Phase B), Vue Dev Server (front), Backend CI Job (PHP + SQLite + Redis), Laravel Test Suite (+5 more)

### Community 94 - "Frontend Components"
Cohesion: 0.17
Nodes (10): flush(), getActiveAlerts, getPublicConfig, getRuntimeConfig, mountAppLayout(), mountedApps, subscribeAlerts, subscribeDeviceStatus (+2 more)

### Community 95 - "Frontend Views"
Cohesion: 0.17
Nodes (9): error, form, loading, router, success, authApi, mountedApps, mountRegisterView() (+1 more)

### Community 96 - "Community 96"
Cohesion: 0.18
Nodes (13): Task 10 Remediation Report, Audit Lifecycle Fixture Fix, Browser Fixture Graph Bootstrap, 44px Minimum Touch Target Remediation, useLabWorkspace Authorization Fix, Task 10 Report — Gate 9/10 Evidence, gate10-quality.yml CI Pipeline, Exact-SHA Browser Evidence (+5 more)

### Community 97 - "Ingestion Service"
Cohesion: 0.39
Nodes (3): DeadLetterStreamService, DeadLetterStreamServiceTest, Mockery

### Community 100 - "Database & Migrations"
Cohesion: 0.26
Nodes (3): ExampleTest, MigrationIntegrityTest, PHPUnit\Framework\TestCase

### Community 101 - "Sensor Management"
Cohesion: 0.30
Nodes (10): boundariesFromRegions(), buildZonesViewModel(), KNOWN_SEVERITIES, moreSevere(), normalizeBoundaries(), normalizeRegions(), normalizeSeverity(), PRECEDENCE (+2 more)

### Community 102 - "Alert Management"
Cohesion: 0.20
Nodes (9): alerts, alertsStore, count, error, loading, flush(), getActiveAlerts, mountActiveAlertsCard() (+1 more)

### Community 103 - "Frontend Views"
Cohesion: 0.17
Nodes (8): exportSensorReadings, getSensor, getSensorLatestReadings, getSensorReadings, mountedApps, mountView(), subscribeSensor, unsubscribeSensor

### Community 106 - "Community 106"
Cohesion: 0.18
Nodes (10): autoload-dev, psr-4, description, license, minimum-stability, name, prefer-stable, Tests\\ (+2 more)

### Community 107 - "Community 107"
Cohesion: 0.31
Nodes (10): ALLOWED_ANON, apiLogin(), authenticatedScenario(), __dirname, guestScenario(), LEAK_ENDPOINTS, main(), OBSERVE_MS (+2 more)

### Community 108 - "Community 108"
Cohesion: 0.18
Nodes (8): backendRoots, compose, __dirname, FRONT, RELAY_TOKENS, REPO, RETIRED_ENDPOINTS, violations

### Community 109 - "Community 109"
Cohesion: 0.18
Nodes (10): dependencies, emailVerification, envExample, missing, missingDependencies, packageJson, requiredDependencies, requiredFiles (+2 more)

### Community 110 - "Frontend Components"
Cohesion: 0.27
Nodes (9): close(), dialogEl, emit, focusableElements(), onKeydown(), props, flush(), mountBaseModal() (+1 more)

### Community 111 - "Device Status & Monitoring"
Cohesion: 0.18
Nodes (7): alertsStore, authStore, canViewAlerts, canViewDevices, deviceStatusesStore, { subscribeAlerts, unsubscribeAlerts }, { subscribeDeviceStatus, unsubscribeDeviceStatus }

### Community 113 - "Alert Management"
Cohesion: 0.20
Nodes (8): error, errors, form, loading, payload(), save(), saving, success

### Community 114 - "Alert Notifications & Email"
Cohesion: 0.18
Nodes (9): adminActions, alerts, diagnostics, email, general, loading, loadNotice, sections (+1 more)

### Community 115 - "Ingestion Service"
Cohesion: 0.38
Nodes (9): PayloadValidationError, Any, Raised when a payload does not satisfy minimum ingestion requirements., validate_payload(), test_validate_payload_accepts_valid_payload(), test_validate_payload_rejects_missing_sensors(), test_validate_payload_rejects_sensor_without_value(), valid_payload() (+1 more)

### Community 116 - "Community 116"
Cohesion: 0.18
Nodes (11): ADR-003: Do Not Move Blade Directly To /front, Blade Views (Legacy UI), script_datos.py (Python IoT Simulator), Operational Risks, Functional State: Blade Legacy, Legacy Views Still Dependent on Blade, Lost or Not-Found Features, Manual Validation Checklist (Not Yet Executed) (+3 more)

### Community 117 - "Alert Management"
Cohesion: 0.20
Nodes (7): getActiveAlerts, transport, channelCallbacks, connectionWatchers, fetchActiveAlerts, listenOnChannel, resyncWatchers

### Community 120 - "Community 120"
Cohesion: 0.24
Nodes (4): AppServiceProvider, ViewServiceProvider, Illuminate\Cache\RateLimiting\Limit, Illuminate\Support\ServiceProvider

### Community 122 - "Community 122"
Cohesion: 0.20
Nodes (10): dependencies, chart.js, laravel-echo, lightweight-charts, pusher-js, laravel-echo, pusher-js, laravel-echo (+2 more)

### Community 123 - "Device Status & Monitoring"
Cohesion: 0.22
Nodes (7): authStore, deviceStatuses, props, getDevices, mountDeviceStatusList(), mountedApps, visibleDevices

### Community 126 - "Community 126"
Cohesion: 0.33
Nodes (8): FALLBACK_TOKENS, hexToRgba(), readCssVar(), resolveChartTokens(), resolveZoneTokens(), ZONE_CSS_VARS, ZONE_FALLBACKS, ZONE_FILL_ALPHA

### Community 127 - "Frontend Views"
Cohesion: 0.20
Nodes (6): error, errors, form, loading, saving, success

### Community 128 - "Device Status & Monitoring"
Cohesion: 0.31
Nodes (4): App\Models\DeviceStatusLog, App\Models\DeviceType, App\Models\Lab, DeviceProvisioningAtomicityTest

### Community 131 - "System Configuration"
Cohesion: 0.22
Nodes (9): pestphp/pest-plugin, php-http/discovery, config, allow-plugins, optimize-autoloader, platform, preferred-install, sort-packages (+1 more)

### Community 132 - "Community 132"
Cohesion: 0.22
Nodes (9): require, laravel/framework, laravel/sanctum, laravel/tinker, laravel/ui, php, predis/predis, pusher/pusher-php-server (+1 more)

### Community 134 - "Community 134"
Cohesion: 0.22
Nodes (8): __dirname, envExample, __filename, forbiddenKeys, frontDockerfile, frontRoot, repoRoot, requiredFiles

### Community 137 - "Sensor Management"
Cohesion: 0.28
Nodes (6): emit, filteredSensors, form, nullableNumber(), props, submit()

### Community 138 - "Frontend Components"
Cohesion: 0.25
Nodes (8): devices, fetchWindow, flush(), mountBoard(), mountedApps, resultForQuery, subscribeSensor, unsubscribeSensor

### Community 139 - "Device Management"
Cohesion: 0.25
Nodes (5): copied, copyError, props, mountCredentialModal(), mountedApps

### Community 140 - "Alert Management"
Cohesion: 0.22
Nodes (6): alert, error, loading, props, resolving, success

### Community 141 - "Sensor Management"
Cohesion: 0.29
Nodes (3): SensorDataController, SensorDataControllerTest, PHPUnit\Framework\Attributes\CoversClass

### Community 143 - "Community 143"
Cohesion: 0.25
Nodes (8): require-dev, fakerphp/faker, laravel/pail, laravel/pint, laravel/sail, mockery/mockery, nunomaduro/collision, phpunit/phpunit

### Community 144 - "Database & Migrations"
Cohesion: 0.25
Nodes (7): DB_CONNECTION, DB_DATABASE, DB_HOST, DB_PASSWORD, DB_PORT, DB_USERNAME, run_mapping_race.sh script

### Community 149 - "Sensor Readings Pipeline"
Cohesion: 0.25
Nodes (4): authStore, props, readingsStore, subscriptions

### Community 150 - "Sensor Readings Pipeline"
Cohesion: 0.25
Nodes (5): mountedApps, mountList(), onReadingBySensor, subscribeSensor, unsubscribeSensor

### Community 151 - "Frontend Realtime Layer"
Cohesion: 0.36
Nodes (6): channelKey(), getChannelRefCount(), leaveChannelName(), listenOnChannel(), refCounts, echoMock

### Community 152 - "Sensor Realtime & WebSocket"
Cohesion: 0.25
Nodes (3): connectionWatchers, echoMock, resyncWatchers

### Community 153 - "Frontend State Management"
Cohesion: 0.36
Nodes (3): activeControllersByConsumer, buildGraphQueryKey(), useGraphSeriesQueryStore

### Community 154 - "Frontend Views"
Cohesion: 0.25
Nodes (6): authStore, error, loading, resending, route, success

### Community 155 - "Database & Migrations"
Cohesion: 0.29
Nodes (8): Phase 8A API Additions (CRUD/Admin), Fase 7B: Operational Validation Before Blade Cleanup, Fase 8A: Admin CRUD/Catalog SPA Migration, QueueSmokeJob (queue infrastructure smoke test), High Risks, Fase 6B: Blade Legacy Cleanup and Production SPA Adjustment (Pending), Fase 8: Final Cleanup (Pending), Session Progress: Fase 8A Work Log

### Community 160 - "Sensor Readings Pipeline"
Cohesion: 0.38
Nodes (7): Authorization Consistency Fix (Phase C), ResourceAccessService, ResourceAccessService, SEC-RT-002 — Sensor Telemetry Authority, SensorDetailView.vue, sensor_reading.view Permission, sensor.view Permission

### Community 161 - "Community 161"
Cohesion: 0.29
Nodes (6): allowScripts, esbuild@0.21.5, name, private, type, version

### Community 162 - "Community 162"
Cohesion: 0.29
Nodes (6): missing, navbar, requiredFiles, root, router, srcFilesToCheck

### Community 165 - "Community 165"
Cohesion: 0.38
Nodes (3): getGraphSeries(), getPrivateGraphSeries(), toWindowParam()

### Community 166 - "Frontend Components"
Cohesion: 0.52
Nodes (4): composeGraphSeries(), computeStats(), idCompare(), toPoint()

### Community 167 - "Device Status & Monitoring"
Cohesion: 0.33
Nodes (5): deviceStatuses, status, statuses, mountedApps, mountRealtimeStatus()

### Community 168 - "Frontend Realtime Layer"
Cohesion: 0.29
Nodes (4): connectionWatchers, echoMock, getDeviceStatusSnapshot, resyncWatchers

### Community 169 - "Frontend Views"
Cohesion: 0.29
Nodes (5): error, form, loading, route, success

### Community 170 - "Database & Migrations"
Cohesion: 0.33
Nodes (7): ADR-007: Docker Per Service, Docker/Docker Compose (absent at audit time), Phase 6 Physical Separation Notes, Phase 7 Docker Consumer Notes, Fase 6: Physical Separation Into /back, Fase 7: Docker and Compose, Functional State: Docker

### Community 171 - "Sensor Readings Pipeline"
Cohesion: 0.29
Nodes (7): Pusher / Laravel Echo Realtime, Broadcast Event: NewAlertTriggered (channel alerts), Broadcast Event: NewSensorReading (channel sensor.{id}), Phase 4 Frontend Consumer Notes, Phase 5 Realtime Contract Notes, Fase 4: Progressive Screen Migration, Fase 5: Realtime (Echo/Pusher) Integration

### Community 172 - "Dashboard"
Cohesion: 0.52
Nodes (7): Task 8 Report — Authenticated Graph Catalog, GET /api/dashboard/graph-catalog Endpoint, DashboardGraphCatalogController, getAuthenticatedGraphCatalog() Frontend Client, Public Bootstrap + Authenticated Catalog Merge, RuleToGraphZones Mapping, Sanctum API Authentication Group

### Community 175 - "Community 175"
Cohesion: 0.33
Nodes (5): private, scripts, build, dev, type

### Community 176 - "Ingestion Service"
Cohesion: 0.47
Nodes (3): Client, Any, MQTTMessage

### Community 177 - "Community 177"
Cohesion: 0.47
Nodes (3): createLogger(), formatTag(), LOG_LEVELS

### Community 178 - "Frontend Views"
Cohesion: 0.33
Nodes (4): email, error, loading, success

### Community 179 - "Frontend Views"
Cohesion: 0.33
Nodes (4): authStore, form, route, router

### Community 180 - "Frontend Views"
Cohesion: 0.33
Nodes (4): error, items, loading, system

### Community 181 - "Frontend Views"
Cohesion: 0.47
Nodes (5): configApi, flush(), mountView(), response(), setResponses()

### Community 182 - "Database & Migrations"
Cohesion: 0.33
Nodes (6): iot-platform-v2 Project, Incremental Migration Principle, Front/Back Separation Goal, ADR-002: Backend As REST API + Broadcasting, ADR-004: Incremental Migration, Laravel 12 / PHP 8.2+ Backend

### Community 183 - "Database & Migrations"
Cohesion: 0.33
Nodes (6): ADR-001: Keep A Monorepo Initially, Migration Hygiene Rules, Fase 0: Read-only Audit and Real Contract, Fase 1: Headless Auth (Sanctum Bearer), Fase 2: API Normalization Replacing Blade, Codex Working Notes Operational Rules

### Community 185 - "Database & Migrations"
Cohesion: 0.40
Nodes (5): autoload, psr-4, App\\, Database\\Factories\\, Database\\Seeders\\

### Community 186 - "System Configuration"
Cohesion: 0.40
Nodes (4): Monolog\Handler\NullHandler, Monolog\Handler\StreamHandler, Monolog\Handler\SyslogUdpHandler, Monolog\Processor\PsrLogMessageProcessor

### Community 187 - "Community 187"
Cohesion: 0.40
Nodes (5): chart.js, chart.js, Functional State: Frontend, Functional Inventory: Frontend SPA Table, Partially Migrated Features

### Community 188 - "Community 188"
Cohesion: 0.40
Nodes (3): chart, lineSeries, lineSeriesDef

### Community 192 - "Database & Migrations"
Cohesion: 0.40
Nodes (5): Front SPA HTML Entrypoint, ADR-008: Frontend Framework (Vue 3 + Bootstrap 5, proposed), Vite (laravel-vite-plugin), Phase 3 Frontend Consumer Notes, Fase 3: Vue 3 SPA Initialization

### Community 193 - "Frontend Components"
Cohesion: 0.50
Nodes (3): metrics, props, renderMetricsCards()

### Community 195 - "Sensor Readings Pipeline"
Cohesion: 0.60
Nodes (3): normalizeReading(), normalizeReadings(), useSensorReadingsStore

### Community 196 - "Community 196"
Cohesion: 0.50
Nodes (5): ADR-009: SPA Auth Uses Sanctum Bearer Tokens Initially, Laravel Sanctum, Authentication Strategy: Sanctum Bearer Tokens, Bearer Token in localStorage Risk, SEC-003: Bearer Token in localStorage

### Community 197 - "Sensor Management"
Cohesion: 0.90
Nodes (5): Task 9 Report — Oversized Graph Window Validation, PublicGraphController, PublicGraphSeriesService, SensorApiController, 24-Hour Window Validation (HTTP 422)

### Community 198 - "Gate Certification Tests"
Cohesion: 0.50
Nodes (4): Dependency Security Fix (Phase A), Composer Audit, Dependency Security CI Job, npm Audit

### Community 200 - "Alert Management"
Cohesion: 0.83
Nodes (3): getAudioContext(), playAlertSound(), unlockAlertSound()

### Community 203 - "Community 203"
Cohesion: 0.67
Nodes (3): extra, laravel, dont-discover

### Community 204 - "Community 204"
Cohesion: 0.67
Nodes (3): keywords, framework, laravel

### Community 205 - "System Configuration"
Cohesion: 0.67
Nodes (3): dev, Composer\\Config::disableProcessTimeout, npx concurrently -c \"#93c5fd,#c4b5fd,#fb7185,#fdba74\" \"php artisan serve\" \"php artisan queue:listen --tries=1\" \"php artisan pail --timeout=0\" \"npm run dev\" --names=server,queue,logs,vite

### Community 239 - "Community 239"
Cohesion: 0.67
Nodes (3): User model, ResetPasswordNotification, VerifyEmailNotification

### Community 240 - "Sensor Readings Pipeline"
Cohesion: 0.67
Nodes (3): allReadings Global Bounds Fix (Phase E), allReadings Global Bounds Bug, SensorMappingService

### Community 241 - "Sensor Management"
Cohesion: 0.67
Nodes (3): Ingestion HTTP Tier — Live Vertical Slice, RawEventOutbox, RawSensorEvent

### Community 242 - "Sensor Readings Pipeline"
Cohesion: 0.67
Nodes (3): iot.domain-events Stream, ReadingProjections, SensorReading

### Community 249 - "Gate Certification Tests"
Cohesion: 0.67
Nodes (3): Frontend CI Job (Vitest + Build + No-Polling), Static No-Polling Gate (AST Scan), Vitest

### Community 250 - "Public API Surface"
Cohesion: 0.67
Nodes (3): ADR-005: Separate Environment Variables, ADR-006: API-Only Frontend Communication, FIX-001: front/.env Configured With Only Public VITE_* Vars

### Community 251 - "Community 251"
Cohesion: 0.67
Nodes (3): Functional State: Backend, Functional Inventory: Backend API Table, Conserved Features (Blade to Vue Parity)

### Community 252 - "Community 252"
Cohesion: 0.67
Nodes (3): Task 11 Report — Mobile/Accessibility, AppLayout Route Stub, Dependency Audit (npm audit --offline)

## Ambiguous Edges - Review These
- `NewSensorReading event` → `AlertService`  [AMBIGUOUS]
  INFORME_ALERTAS_NOTIFICACIONES.md · relation: calls

## Knowledge Gaps
- **762 isolated node(s):** `DashboardPreferenceController`, `backendRoots`, `compose`, `__dirname`, `FRONT` (+757 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 1483 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **113 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **What is the exact relationship between `NewSensorReading event` and `AlertService`?**
  _Edge tagged AMBIGUOUS (relation: calls) - confidence is low._
- **Why does `User` connect `Auth & Security Tests` to `Auth & Security Tests`, `Sensor Readings Pipeline`, `Sensor Readings Pipeline`, `Sensor Readings Pipeline`, `Sensor Readings Pipeline`, `Sensor Readings Pipeline`, `Sensor Readings Pipeline`, `Sensor Readings Pipeline`, `Sensor Readings Pipeline`, `Sensor Management`, `Sensor Readings Pipeline`, `Sensor Management`, `Community 158`, `Community 159`, `Sensor Management`, `System Configuration`, `Ingestion Service`, `Alert Rules Engine`, `Community 78`, `Sensor Readings Pipeline`, `Community 98`, `Gate Certification Tests`?**
  _High betweenness centrality (0.045) - this node is a cross-community bridge._
- **Why does `TestCase` connect `Auth & Security Tests` to `Auth & Security Tests`, `Sensor Readings Pipeline`, `Sensor Readings Pipeline`, `Sensor Readings Pipeline`, `Sensor Readings Pipeline`, `Sensor Readings Pipeline`, `Sensor Management`, `Sensor Readings Pipeline`, `Community 146`, `Gate Certification Tests`, `Community 148`, `Ingestion Service`, `Community 158`, `Community 159`, `Dead Letter & Outbox`, `Sensor Readings Pipeline`, `Sensor Management`, `Sensor Management`, `Sensor Management`, `Ingestion Service`, `Dead Letter & Outbox`, `Ingestion Service`, `Community 190`, `Community 191`, `Community 78`, `Dead Letter & Outbox`, `Dead Letter & Outbox`, `Device Status & Monitoring`, `Ingestion Service`, `Community 98`, `Gate Certification Tests`, `Sensor Readings Pipeline`, `Device Status & Monitoring`?**
  _High betweenness centrality (0.044) - this node is a cross-community bridge._
- **Why does `Sensor` connect `Sensor Readings Pipeline` to `Auth & Security Tests`, `Auth & Security Tests`, `Sensor Readings Pipeline`, `Sensor Readings Pipeline`, `Sensor Readings Pipeline`, `Sensor Readings Pipeline`, `Sensor Readings Pipeline`, `Sensor Readings Pipeline`, `Sensor Readings Pipeline`, `Sensor Management`, `Sensor Readings Pipeline`, `Sensor Readings Pipeline`, `Sensor Readings Pipeline`, `Community 146`, `Sensor Management`, `Sensor Readings Pipeline`, `Sensor Management`, `Ingestion Service`, `Sensor Management`, `Community 158`, `Community 159`, `Sensor Management`, `Dead Letter & Outbox`, `Sensor Readings Pipeline`, `Sensor Management`, `Sensor Management`, `Sensor Readings Pipeline`, `Community 191`, `Dead Letter & Outbox`, `Gate Certification Tests`, `Sensor Readings Pipeline`, `Alert Management`?**
  _High betweenness centrality (0.031) - this node is a cross-community bridge._
- **What connects `DashboardPreferenceController`, `backendRoots`, `compose` to the rest of the system?**
  _762 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `Auth & Security Tests` be split into smaller, more focused modules?**
  _Cohesion score 0.0326221125022042 - nodes in this community are weakly interconnected._
- **Should `Auth & Security Tests` be split into smaller, more focused modules?**
  _Cohesion score 0.03248693054518297 - nodes in this community are weakly interconnected._