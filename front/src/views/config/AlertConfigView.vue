<template>
  <section class="lab-resource-page settings-page">
    ...
    <form v-else class="settings-form-panel" @submit.prevent="save">
      ...
      <div class="row g-3">
        <div class="col-12 col-lg-6"><BaseInput v-model="form.alert_threshold" label="Umbral general" name="alert_threshold" type="number" :error="fieldError('alert_threshold')" required /></div>
        <div class="col-12 col-lg-6"><BaseInput v-model="form.danger_email_rate_limit_seconds" label="Límite de correo crítico (s)" name="danger_email_rate_limit_seconds" type="number" :error="fieldError('danger_email_rate_limit_seconds')" required /></div>
      </div>
...
const form = reactive({ mail_enabled: false, alert_sound_enabled: false, alert_threshold: 0, danger_email_rate_limit_seconds: 0 });
...
function payload() { return { mail_enabled: Boolean(form.mail_enabled), alert_sound_enabled: Boolean(form.alert_sound_enabled), alert_threshold: Number(form.alert_threshold), danger_email_rate_limit_seconds: Number(form.danger_email_rate_limit_seconds) }; }
