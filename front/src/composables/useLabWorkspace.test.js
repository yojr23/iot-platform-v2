...
const apps=[];
async function mount(authenticated=false,admin=true,initialDevices=[{id:1,name:'Device',sensors:[{id:1,name:'Temperature',unit:'C'},{id:2,name:'Pressure',unit:'bar'}]}]){let state;const pinia=createPinia();const auth=useAuthStore(pinia);const devices=ref(initialDevices);if(authenticated){auth.token='test';auth.user={id:1,is_admin:admin};}const app=createApp({setup(){state=useLabWorkspace(devices);return()=>null;}});app.use(pinia);app.mount(document.createElement('div'));apps.push(app);await nextTick();await Promise.resolve();return {state,devices};}
...
describe('Lab workspace explicit persistence',()=>{
 ...
 it('does not persist dashboard edits for an authenticated standard user',async()=>{const {state:s}=await mount(true,false);s.changeRange('1h');await s.save();expect(s.canManageDashboard.value).toBe(false);expect(api.save).not.toHaveBeenCalled();});
 it('keeps dashboard editing available for an authenticated administrator',async()=>{const {state:s}=await mount(true,true);expect(s.canManageDashboard.value).toBe(true);});
 it('prevents a guest draft from being saved to authenticated preferences',async()=>{const {state:s}=await mount();s.remove(s.selectedId.value);await s.save();expect(api.save).not.toHaveBeenCalled();expect(s.saveState.value).toBe('dirty');});
});
