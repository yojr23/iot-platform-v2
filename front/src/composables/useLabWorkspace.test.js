...
const apps=[];
async function mount(authenticated=false,initialDevices=[{id:1,name:'Device',sensors:[{id:1,name:'Temperature',unit:'C'},{id:2,name:'Pressure',unit:'bar'}]}]){let state;const pinia=createPinia();const auth=useAuthStore(pinia);const devices=ref(initialDevices);if(authenticated){auth.token='test';auth.user={id:1,is_admin:true};}const app=createApp({setup(){state=useLabWorkspace(devices);return()=>null;}});app.use(pinia);app.mount(document.createElement('div'));apps.push(app);await nextTick();await Promise.resolve();return {state,devices};}
...
describe('Lab workspace explicit persistence',()=>{
 ...
 it('keeps dashboard editing available for a guest temporary draft',async()=>{const {state:s}=await mount();expect(s.canManageDashboard.value).toBe(true);});
 it('keeps dashboard editing available for an authenticated user',async()=>{const {state:s}=await mount(true);expect(s.canManageDashboard.value).toBe(true);});
