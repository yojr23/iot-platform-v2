...
describe('Lab workspace explicit persistence',()=>{
 ...
 it('exposes canManageDashboard as false for non-admin users',async()=>{const {state:s}=await mount();expect(s.canManageDashboard.value).toBe(false);});
 it('exposes canManageDashboard as true for admin users',async()=>{const {state:s}=await mount(true);s.auth.user={id:1,is_admin:true};expect(s.canManageDashboard.value).toBe(true);});
 it('prevents non-admin users from saving',async()=>{const {state:s}=await mount();s.remove(s.selectedId.value);await s.save();expect(api.save).not.toHaveBeenCalled();expect(s.saveState.value).toBe('dirty');});
});
