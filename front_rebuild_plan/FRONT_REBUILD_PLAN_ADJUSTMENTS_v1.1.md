...
> **Commit-review correction, 9 September 2026:** `PRE_GATE6_COMMIT_REVIEW_2026-09-09.md` records the reviewed preflight range through `00b560c`. Added blockers: private-channel tests must expect `private-sensor.{id}`; `RawReadingNormalizer` is in the timestamp blast radius; alert polling/subscription/count/toast state must be torn down on logout and started on login; authenticated sensor realtime still needs a private-channel Vue path; `NewSensorReading` must become explicit-audience/fail-closed; `/config/public` needs a protected runtime replacement before removal; and remaining Blade sensor CRUD needs an auth-continuity decision. The legacy anonymous routes and broad sensor events are transitional, not compliant public-graph end state: Stage 6 still must apply `PublicGraphVisibility`, replace routes with graph bootstrap/series, reduce the public event, and prove the final route inventory.
...
Skill provenance correction: the external marketplace graph did not enumerate every requested review style, but repository history includes `.opencode/skills/ponytail` and `.opencode/skills/zero-hallucination-coder` before `44f9254` removed tracked skills. This plan applies those controls directly: one small end-to-end vertical slice before expansion, one owner per concern, no fallback duplication, and no assertion beyond repository evidence.
...
4. **Replace the legacy monitor reading model with one scoped graph data path.** The live Pinia reading projection is keyed by `sensorId` and owns sample normalization, validation, deterministic `reading_id`/timestamp/sequence merge, bounded memory, latest value, and last-observed presentation. A separate historical query layer is keyed by authorization context, sensor ID, resolved UTC window, and aggregation; it owns cancellation, current-request identity, range hydration, and source-set statistics. `channelRegistry`/`useSensorRealtime`, not Pinia, owns Echo channel selection, ref-count, lifecycle recovery trigger, subscription release, and public/private channel switching. Both layers must clear data from a revoked authenticated scope after logout.
