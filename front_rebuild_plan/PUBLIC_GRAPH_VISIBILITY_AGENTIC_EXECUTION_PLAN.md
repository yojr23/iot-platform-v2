...
1. **Verify the already-selected canonical dashboard entry (RESOLVED in `00b560c`).** The choice is closed: `FRONT_URL/dashboard → Vue SPA`. Assert, do not re-choose: `GET /` and `GET /dashboard` redirect to `config('app.front_url').'/dashboard'`; no broad Blade dashboard exists (`DashboardController.php` and `dashboard.blade.php` are deleted). Do not add `back/app/Http/Controllers/DashboardController.php` or `back/resources/views/dashboard.blade.php` to any change map — they no longer exist.
...
| Point semantics | Raw point `{ timestamp, value, reading_id }` (reading_id integer). Aggregated point `{ timestamp, min, max, mean, sample_count, reading_id: null }`. The exact aggregation interval/mode (e.g. 30s / 1m / 5m / adaptive) is selected from measured evidence in Stage 6, not frozen here. Response includes original valid-sample min/max/mean/count and gap_encoding: absent-bucket. | Never synthesize zero/null samples for gaps. V1 has no threshold, quality, cadence, coverage, precision, stale, or device-status field. |
...
| Public-entry and browser delivery | back/routes/web.php; back/app/Services/Ingestion/DomainEventBroadcastConsumer.php; back/app/Console/Commands/ConsumeDomainEvents.php; back/app/Events/NewSensorReading.php; back/tests/Unit/EventEnvelopeTest.php | The broad anonymous Blade dashboard is **already retired** in `00b560c` — `DashboardController.php`, `dashboard.blade.php`, and `sensors/index.blade.php` are deleted; do not treat them as owners to modify. Remaining Stage 6 work: consumer applies `PublicGraphVisibility` after durable publication and before minimal dispatch; retained authorized views derive labels/unit locally. |
...
**Files:** `back/routes/web.php`, `back/routes/api.php`, `back/app/Http/Controllers/Api/ConfigController.php`, a migration for the graph range index, and focused feature/route tests. (`DashboardController.php` and the Blade dashboard/sensor read views are already deleted in `00b560c` — not editable owners.)

- [ ] Verify (do not re-select) the canonical guest entry: assert `GET /` and `GET /dashboard` redirect to `config('app.front_url').'/dashboard'` and that no broad Blade dashboard response exists (already retired in `00b560c`). Capture the current anonymous route table for the inventory.
...
- [ ] Verify the broad public Blade dashboard is already retired (`00b560c`); for any authenticated legacy page still retained, prove it renders a unit without `NewSensorReading.unit`.
...
Include public-offline inclusion, guessed-ID 404, exact-window filtering, raw ordering, aggregated-bucket behavior (for whichever interval Stage 6 measures), absent bucket gaps, invalid UTC/range/aggregation 422, and raw count-bound tests.
