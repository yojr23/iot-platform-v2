...
Guests and authenticated users share the same graph engine, realtime infrastructure, chart primitives, graph semantics, tokens, and responsive visual language. They may use different page compositions. Guest mode is the reduced public graph composition; authenticated mode is the broader operational workspace. Guests do not receive a legacy dashboard, a login wall before graph monitoring, or a separate realtime implementation.
...
Skill provenance correction: the external marketplace graph did not enumerate every requested review style, but repository history includes `.opencode/skills/ponytail` and `.opencode/skills/zero-hallucination-coder` before `44f9254` removed tracked skills. Their requested intent is preserved as implementation discipline: one small vertical slice first, one owner per responsibility, no public compatibility fallback, and no capability asserted without repository evidence.
