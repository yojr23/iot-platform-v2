...
**Derives from:** `audit.md` (audited SHA `880cffbcfc7aecb081fb378642d8693c1df64170`, re-confirmed current). The audit is the source of truth for *why* and *what's broken*; this document is the ordered, gated *how* — the plan to fix every finding and reach both Definitions of Done (`audit.md §88` mobile, `§89` event architecture).

**Chosen architecture:** the **full** target from `audit.md §17–§19` — transactional outbox + binlog-driven CDC relay, a dedicated `iot.domain-events` stream, per-consumer-group DLQ, and the complete cursor/replay/snapshot recovery protocol. (A leaner variant that defers CDC/DLQ-stream/6-step-recovery to Laravel queue + `failed_jobs` + one-shot snapshot is documented in `audit.md §15a`; it is **not** the path taken here, but remains the cost-down fallback if a deployment feasibility gate fails.)
...
**Golden rules gating every realtime stage (`audit.md §86`, §91):** NO polling · NO polling fallback · NO hybrid realtime · NO periodic state discovery · NO browser→Redis · NO synchronous RPC disguised as events · NO unversioned durable events · NO assumption of single delivery.
...
Audit roadmap gates `G0, P1…P10` (`audit.md §19`) map 1:1 to Stages 0–10 below; `TASK-001…012` (`§20`) are cited per stage.
...
- **0.1** Fix `front/.audit-e2e/fixtures.mjs` field-name mismatch (`/dashboard/public` returns `{devices,sensors,alerts}` counts; the app reads `total_devices`/`active_devices`/`total_sensors`/`active_alerts`/`unresolved_alerts` and expects a device **array**). Re-run dashboard 320/390 with real data → unblocks **M03, M07** evaluation.
- **0.2** Add interaction steps to the harness: open device + alert-rule modals, inject a long/critical `AlertTriggered` to fire the toast, `getBoundingClientRect()` on `btn-sm` controls → unblocks **M04, M05, M06**.
- **0.3** Wire the `long`/`dense` fixture modes (already stubbed) into the matrix for tables + admin headers → unblocks **M02**.
- **0.4** Build the **event-injection harness** (`audit.md §69`): inject `SensorReadingCreated`/`AlertTriggered`/`AlertResolved`/`DeviceStatusChanged` at the frontend event-adapter boundary, label results `EVENT_HANDLER_SIMULATED` (not transport-verified). This is the reusable rig every realtime stage verifies against.
- **0.5** Add the **no-polling network assertion** rig (`audit.md §71`): observe network ≥3× the longest current timer interval, assert no recurring latest-state REST after cutover. Green now (timers still present) = baseline; it becomes the pass/fail gate for Stages 6/7.
...
No backend dependency — ship for immediate value while Stages 2–5 proceed.

- **1.1 M01 + M02 + M09 (shared root):** add a mobile-visible `/profile` entry to `NavBar.vue`'s collapsed menu (remove `d-none d-md-inline` on line 48 / add to `navItems`); apply `flex-wrap`/stacking to the un-wrapped `d-flex justify-content-between` page headers (`DevicesView.vue:3–14`, `SensorsView.vue`, siblings the `991.98px` rule in `main.scss:253` misses). The header is the source-verified likely M09 cause — fix it, then re-run `devices-admin` @320×700 (0.4/0.5 harness) to confirm `hasOverflow:false` before touching the table itself.
...
**Done when:** `audit.md §88` passes at tested widths, desktop unregressed, all harness-confirmed.
...
- **2.2 Canonical versioned envelope** (`event_id, event_type, event_version:1, occurred_at, producer, aggregate_type/id, aggregate_version, correlation_id, causation_id?, bounded payload` — `audit.md §8/§48`). Apply to the three existing events; keep current names as backward aliases during rollout.
...
- **2.4 Idempotency keys at the schema level:** unique constraints for raw `(source, source_event_id)` and alert `(sensor_reading_id, alert_rule_id)` — the check-then-create in `AlertService.php:51–70` has a real race with no DB backstop.

**Done when:** mixed-version producers/consumers coexist; malformed payloads rejected, additions tolerated; private events denied to unauthorized users; public guest dashboard still works. (`audit.md §49/§58`.)

---

## Stage 3 — Durable ingestion: outbox + CDC relay + raw consumer (audit P3 / TASK-004, TASK-005)

The reliability core. Full §19 version.

- **3.1 Transactional outbox** (`audit.md §66`): producer-generated event identity + business row + outbox row in one DB transaction. Closes the dual-write gap where `IngestionController` returns 201 even when `XADD` fails.
- **3.2 Binlog-driven CDC relay** (`audit.md §15 decision, §18`): route typed outbox rows to `iot.raw-events`, durable checkpoint advanced only after accepted delivery; survives Redis loss. **Gate:** requires a deployment-feasibility proof (MySQL binlog → Redis sink, offset persistence) — `audit.md` open questions #6/#9. If that gate fails, fall back to the `§15a` queue-based relay.
...
**Done when:** DB/Redis crash matrix (before/after commit and ack) redelivers safely; receipt cannot be silently stranded; quiet-stream pending recovers; one reading + intended alert per logical event across redelivery; DLQ + replay work. (`audit.md §45/§65/§67`.)

---

## Stage 4 — Domain event model + centralized transitions (audit P4 / TASK-006)

- **4.1** Stand up `iot.domain-events` (CDC relay from a domain outbox) with independent consumer groups `browser-delivery-v1`, `email-delivery-v1`, `webhook-delivery-v1(gated)` — `audit.md §18`. Justified over raw-only because raw data can't reconstruct manual resolution/device mutations (`§47`).
...
- **4.3** Move transition ownership from observers to an explicit service (`audit.md §15/§53`): observers keep only well-defined lifecycle behavior, never sole coverage for bulk writes; broadcasting becomes a durable async consumer, not `ShouldBroadcastNow` on the request path.
...
**Done when:** 1/3/5 charts with duplicate/out-of-order events + 60-point cap hold; 390→320→768→390 resize clean; **the §71 network assertion shows zero periodic latest-readings.**
...
**Done when:** create / duplicate / resolve-twice / bulk-resolve / another-session / reconnect all agree; **§71 assertion shows zero periodic active-alert requests.**
...
- **10.3 Prove the no-polling gate deployment-wide:** source inspection + browser network traces + transport restriction + backend relay/worker inspection. A passing build or simulated handler is insufficient (`audit.md §89`).

**Done when:** `audit.md §89` (Definition of Done — Event Architecture) holds end to end: all polling + fallback removed, consumer pipeline complete with XACK-after-commit, pending recovery + bounded retry + DLQ, versioned contracts, idempotent consumers, one Echo per tab, event-driven sensor/alert/device, event-driven recovery, async signed webhooks + async email, observability live, browser never touches Redis.
...
Stage 0 (evidence baseline) ─┬─► Stage 1 (mobile blockers) ──────────────► ship independently (frontend-only)
                             ...
                             └─► Stage 2 (contracts + broadcasting scaffolding)
                                    └─► Stage 3 (outbox + CDC relay + raw consumer)   ⟵ CDC feasibility gate here
...
Stages 1 and (0→2→3→4→5) run in parallel — no shared files. Stages 6/7/8 can parallelize once Stage 5 lands. Only 6/7/10 delete a timer/route, and only after recovery + durable delivery are proven.

## Risk gates that can change this plan (`audit.md §22, §24`)

- **CDC feasibility (Stage 3.2):** if a MySQL-binlog→Redis relay with durable checkpoints can't be operated (open Q #6/#9), drop to the `§15a` lean relay (Laravel queue + outbox) — same acceptance, less infra.
