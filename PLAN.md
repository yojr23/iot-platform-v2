...
> **Session status, 9 September 2026 — Stage 6/7/8 started.** Full evidence: `front_rebuild_plan/STAGE_6_7_8_SESSION_EVIDENCE_2026-09-09.md`. **DONE (code+tests, uncommitted, GAP=run-machine):** Stage 6.0 server-owned graph boundary (`PublicGraphVisibility` fail-closed sole owner + `public_monitoring_enabled` column + `/api/public/graph/{bootstrap,series}` + consumer visibility gating); Stage 6.1 DRY chart owner; Stage 6.2 live projection + graph-series recovery + **sensor polling deleted** in `SensorMonitorBoard.vue`; Stage 7 **backend** (`/api/alerts/active` → `auth:sanctum`, alert events → `PrivateChannel('alerts')`); Stage 8.2 email off the sync path + rate-limit-release fix. Pre-gate P1 fixes also landed: sensor realtime private/public by stored token (race fixed), `NewSensorReading` fail-closed, PAT `/broadcasting/auth` test. **STILL OPEN:** P0 `.env.host-backup` purge + credential rotation (blocked by operator push-hold); Stage 7 **frontend** (private alerts channel + store idempotency + delete `AppLayout`/`ActiveAlertsCard` timers); `GET /api/config/runtime`; Stage 8.1 device frontend projection; legacy Blade alert regression (expected — deferred retirement). `PublicGraphVisibility` **is** now wired into public delivery; the earlier "not yet wired" / "expect `private-sensor.{id}`" / `00b560c` commit-review notes are resolved.
