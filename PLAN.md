...
- **6.2 Scoped sensor-reading projection:** wire selected approved graph sources through the event adapter → one Pinia projection → shared chart owner. Key projection state by authorization context, sensor, resolved UTC window, and aggregation; own request generation/cancellation, normalization, deterministic ID/timestamp/sequence merge, bounded memory, recovery, and release. Clear inaccessible state on logout/scope revocation. Guest and authenticated graph areas must have visual parity for the Lab Blue graph workflow—same shell, toolbar, selectors, range controls, chart hierarchy, and empty/error treatment—while capability-only slots may use an access-required/omitted variant; do not require a shared page tree.
