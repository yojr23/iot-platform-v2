...
- Current repository HEAD: `4ab5e3f` — `docs(evidence): record final Mac live certification on 4b14825`. **Application-code freeze SHA: `4b14825`** (`fix(ingestion): normalize RawSensorEvent received_at to app timezone`); every commit after it is test-harness / evidence / docs only. The full live certification below was re-run to evidence on `96d9523`/`76f12c9` (byte-identical application source to `4b14825`). Prior authoritative app tip was `f119697`; the 2026-09-19 Mac closure work sits on top of it.
- **Final CI (M25): GREEN.** GitHub Actions `gate10-quality` run **#70** on pushed commit `4ab5e3f` — all 6 jobs PASS (backend, ingestion, architecture, frontend, frontend-responsive-e2e-mocked, dependency-security), 2m 22s. The run corresponds to the final committed repository state.
- Application SHA vs docs/Graphify SHA (kept separate per `docs/graphify-strategy.md`): the last Graphify/docs regeneration is `672421b`. Graphify regeneration against the final application SHA is done in a separate `chore(graphify)` commit (M22).
- Execution environment: **Mac** (Darwin 23.5.0), PHP 8.5.2 (Docker back: PHP 8.4), MySQL 8.0/8.4 (Docker), Redis 7 (AOF), Debezium Server 3.5.2, Node 18.20.8, Docker 27.3.1, Mosquitto 2.1.2 (host, persistent sessions), Laravel Reverb. Backend/ingestion suites and the full live Docker topology (db, redis, back, reverb, debezium, raw/outbox-cdc/domain consumers, ingestion) were executed here directly against real MQTT + WebSocket.
- **Gate 9: CLOSED.** Production preview build (`npm run build` + `npm run preview` on `127.0.0.1:4173`) passed the 35-row mocked Chromium responsive matrix (`audit:gate9`) 35/35 across 320/360/390/768/1024/1280/1440, and the live-stack screenshot pass (M16, `front/.audit-e2e/production-screenshots.mjs`) captured 98 PNGs across those widths × shipped routes × guest/user with **0 auto-detected findings** (no horizontal overflow, no console/page errors). Manual inspection of desktop 1440 + mobile 320/360/390 (dashboard, alerts, sensor detail) confirmed clean responsive layout, with a live MQTT reading rendering in the sensor-detail table.
- **Gate 10: CLOSED.** Live A–E deterministic fault matrix PASSES 5/5 on the live Docker topology (evidence `.audit-e2e/results/gate10-faults-96d9523e9b3d7eef8dd4d88b4ae8292f2684ee7b.json`, doc `docs/implementation/gate10-fault-matrix-current.md`): durable progress across `back` stop (A), XADD-before-XACK reclaim (B), Redis-outage durability (C), poison→DLQ (D), dead-consumer XAUTOCLAIM reclaim (E). Live no-polling network assertion PASSES (`front/.audit-e2e/results/gate10-live-network.json`). **Continuous MQTT→browser E2E PASSES** (`front/.audit-e2e/mqtt-to-browser-live.mjs`): one MQTT publish renders in the browser in ~2.6s through the full mosquitto→ingestion→spool→CDC→reading→domain→Reverb→WS→Echo→Pinia→DOM lineage, no refresh, no recurring REST GET. **H5 real-browser duplicate-delivery PASSES** (`front/.audit-e2e/h5-broadcast-crash-live.mjs`): a post-broadcast worker crash → XAUTOCLAIM re-broadcast delivers 2 physical frames (same reading id 74, event_id 44) → exactly 1 logical table row, XPENDING 0, no DLQ growth — at-least-once transport + stable identity + idempotent projection (NOT exactly-once).
- **MQTT durability (M10): CLOSED with a live broker.** Subscriber-down: 10 QoS1 events published while ingestion offline → restart with identical client id (persistent session, `clean_session=false`) → **10/10 recovered, 0 lost, 0 logical duplicates** (10 distinct raw events). Spool-crash: event reached the SQLite spool while the backend was down → process killed before delivery → after restart the event delivered and the spool row cleared only after acknowledged backend acceptance. Evidence `.audit-e2e/results/mqtt-durability-<sha>.json`.
- Static gates (M7/M20 on final source): backend `php artisan test` **482 passed / 52 skipped** (includes the new `RawSensorEventReceivedAtTimezoneTest`); ingestion `pytest` **49 passed**; frontend `vitest` **341 passed**; `composer audit` and `pip-audit` clean; `npm audit` shows 4 dev-only advisories (vite/esbuild/vitest/@vitest/mocker — dev-server path traversal, never in the production bundle), accepted as documented dev-only residual. `pint --test` reports pre-existing repo-wide style debt in ~130 unmodified files; no new violations introduced this session.
- MySQL data-integrity (M8): fresh `migrate:fresh --seed` on real MySQL OK; the real-MySQL concurrent mapping race held the exactly-one-open-mapping invariant **20/20 iterations** (`tests/concurrency/run_mapping_race.sh`).
- Blade disposition: **retired.** `back/routes/web.php` exposes only SPA-serving GET routes for product screens (no product write routes); one canonical service owner per domain mutation (BACK-01 closed). Reverb: **origins restricted** (env-driven `REVERB_ALLOWED_ORIGINS`, no wildcard — EVT-01) with a `fsockopen` healthcheck and `domain-event-consumer` waiting on `service_healthy` (EVT-02). `MQTT_CLEAN_SESSION` explicit in Compose (EVT-03). Danger-email: bounded retry `tries=3`+backoff, delivery-failure-only throw (EVT-04). Dead `broadcastNewAlert` removed (EVT-05). FRONT-01/02 closed (generation-safe `loading`, alert/rule continuation tests).
- **Two source defects fixed this session, both found only by real live certification (never observable in the mocked suites):** (1) `3dc771b` — the Gate-10 fault harness grepped `docker logs` stdout while the checkpoint marker is on stderr, so scenarios B/E had never passed to committed evidence. (2) `4b14825` — `RawSensorEvent.received_at` stored UTC digits that the `datetime` cast read back as APP_TIMEZONE, pushing every MQTT-sourced `reading_time` ~5h into the future so the browser's clock-drift guard dropped it and **no live telemetry ever rendered**; fixed with an app-timezone setter + regression test.
- PLAN: **CLOSED** on application freeze `4b14825`. See §33 (M24) Definition of Closed — all conditions met; the only residuals are the accepted dev-only `npm audit` advisories and pre-existing Pint style debt in unmodified files.

Exactly one section below is authoritative for current release state: **this block plus the RECONCILIATION — 2026-09-18 (Mac closure)** immediately following. Every other RECONCILIATION/ledger/status section further down (including the earlier Windows one) is **HISTORICAL RECORD — NOT CURRENT RELEASE STATE**, retained for audit traceability only.
...
### STILL OPEN (as of the 2026-09-18 `f119697` snapshot) — ALL SUBSEQUENTLY CLOSED on `4b14825`, see the authoritative header above

> HISTORICAL: every row below was closed in the 2026-09-19 live certification. Retained only to show what the `f119697` snapshot still owed. Do not read these as current OPEN items — the authoritative header is the current state.

| # | Item | `f119697` status | Closure on `4b14825` (live) |
|---|------|------------------|------------------------------|
| 11 | Gate 10 A–E live fault matrix | was OPEN | **CLOSED** — 5/5 PASS, evidence `gate10-faults-96d9523….json` (after fixing the harness stderr bug `3dc771b`). |
| 12 | Continuous MQTT→browser E2E | was OPEN (harness unbuilt) | **CLOSED** — `front/.audit-e2e/mqtt-to-browser-live.mjs` built; one publish → rendered DOM in ~2.6s, no polling. Surfaced + fixed the `received_at` timezone defect (`4b14825`). |
| 13 | Live no-polling network assertion | was OPEN | **CLOSED** — `gate10-live-network.json` PASS on the live Reverb stack. |
| 14/15 | Gate 9 production responsive matrix + visual review | was OPEN | **CLOSED** — 35/35 mocked matrix + 98 live-stack screenshots, 0 auto findings, manual spot-check clean. |
| 16 | Vite/esbuild dev-dep advisory | was OPEN | **ACCEPTED** — 4 dev-only advisories, never in the production bundle; documented dev-only residual. |
| 17/24 | Graphify regeneration | was OPEN | Regenerated against the final application SHA in a separate `chore(graphify)` commit (M22). |
| H5 | Real-browser duplicate-delivery | (not listed) | **CLOSED** — `front/.audit-e2e/h5-broadcast-crash-live.mjs`, 2 physical frames → 1 logical effect. |
| M10 | Live MQTT durability | (not listed) | **CLOSED** — subscriber-down 10/10, spool-crash recovery, live Mosquitto. |
...
- **GATE 10 LIVE no-polling: PASS** (real stack, 60s window, no mocks). Guest: 2 API calls total (only the two allowed public endpoints), 0 leaks, 0 recurring offenders. Authenticated: quiet steady state, bounded recovery (visibility burst 2, reconnect 0), logout `staleTokenStatus=401`. Evidence `docs/evidence-gate10-live-network.json`. Getting a truthful result required fixing two real harness bugs: login read `data.token` vs the real `access_token`; and the recorder miscounted Vite `/src/api/*.js` module fetches and self-flagged an allowlisted public endpoint as a leak.
- **Durable self-contained event (audit P1/M3) — CLOSED (`127affc`):** `sensor.reading.created` outbox payload now carries the denormalized sensor/device/lab fields **and** `public_at_occurrence` (event-time audience). `DomainEventBroadcastConsumer` delivers straight from the payload and, if the `SensorReading` row is gone by delivery time, rebuilds an in-memory reading from the payload — no silent loss, no mandatory re-read. This settles the audit's event-time-vs-delivery-time visibility question in favor of **event-time** and closes the ⚠ leak-risk concern noted for the earlier payload-only self-containment (audience is captured at write time, not re-derived at delivery). Legacy payloads fall back to find()+isPublic. Suite 520 pass / 4 skip (with Redis).
...
- **Full MQTT→broadcast vertical (live, complete stack):** one correlation id (`mac-e2e-...`) traced through mosquitto → Python spool → HTTP → RawSensorEvent+outbox → Debezium binlog CDC → Redis → cdc-consumer → raw-consumer → normalizer → SensorReading → domain outbox (`delivered=yes`), payload enriched (`public_at_occurrence`, `sensor_name`). Final browser frame covered by the GATE 10 LIVE capture (local `BROADCAST_CONNECTION=log`).
- **Event-recovery fault matrix (live, zero-loss):** H1 Debezium restart (resumed from durable Redis offset, during-restart event fully processed); H2 Redis restart (streams persisted, consumers reconnected, event recovered); H3 XAUTOCLAIM (replacement worker reclaimed an orphaned pending entry, pending 1→0); H4 poison→DLQ.
...
| Live Gate 9/10 evidence | P0 | ⚠️ PARTIAL (2026-09-17) | Now done: **live 60s no-polling browser capture PASS**; **full-stack MQTT→broadcast vertical trace** (mosquitto+debezium+CDC+consumers, one correlation id through every backend boundary); **event-recovery fault matrix** (H1 Debezium restart, H2 Redis restart, H3 XAUTOCLAIM takeover, H4 poison→DLQ — all zero-loss); durable self-contained reading event; ingestion HTTP tier; MySQL EXPLAIN; dependency audit; MySQL mapping concurrency. Still open: single continuous MQTT→browser frame (needs soketi vs `BROADCAST_CONNECTION=log`), broadcast crash windows (H5), desktop/mobile visual QA, branch protection. See `docs/mac-certification-evidence.md`. |
