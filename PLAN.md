...
**Derives from:** `audit.md` (original audit SHA `880cffbcfc7aecb081fb378642d8693c1df64170`) plus a required application-code refresh against baseline `06d619c4c57c2cacae0c890e4dd7056097ddc79d`. Current planning-document base HEAD is `905dd1f` (commit `ajustes de seguridad`). Future documentation-only commits should not be mistaken for application drift. The audit is the source of truth for *why* and *what's broken*, but it is not yet fully current/reproducible. This document is the ordered, gated *how* to fix every finding and reach the Definition of Done in `audit.md §23`.
...
## CURRENT STATUS — Gates 6/7/8 code-complete (2026-09-11)
...
- **GATE 9 — near-closed (2026-09-11).** Playwright matrix 53/53 clean across the DoD viewport set (320/360/390/768/1024/1280/1440), guest+authenticated, incl. the redesigned `/metrics`. Remaining before CLOSED: exhaustive long/empty/dense + modal/auth-transition matrix re-run against this HEAD (subset covered; not the full matrix).
- **GATE 10 — substantially closed, live pipeline VERIFIED (2026-09-11).** Backend suite **303 tests, 0 failed, 0 skipped (1170 assertions)**; 26 CDC/domain-event tests run on real Redis; G10-PUB-08 green; CI now provisions Redis. **Full CDC delivery pipeline verified end to end on the live Docker stack** (ingest → MySQL binlog → Debezium → Redis CDC stream → outbox-cdc-consumer → `iot.raw-events` → raw-consumer → 2 `sensor_readings` → domain outbox → domain-event-consumer delivered 2/2). Two real bugs found+fixed bringing the stack up: (a) Debezium 3.5 config mount path (`/debezium/config/` not `/conf/`); (b) **Redis stream key-prefix mismatch** — publishers used the prefixing Redis facade while consumers/Debezium read raw unprefixed keys, so nothing was ever delivered in the running stack (the 26 unit tests use fake publishers and could not catch it). Fixed both publishers to raw unprefixed XADD; frozen by `PublisherStreamPrefixTest` (real Redis) + rewritten `EventPublisherXaddShapeTest`. Private-sensor history feature verified in a **real browser at desktop 1440 + mobile 390** (adds a restricted-sensor graph → 1 call to `/api/sensors/{id}/series`, 0 to public). **Still open before CLOSED:** (1) scripted A–E fault-injection runs (happy path done, faults not); (2) formal `network-assertion-live.mjs` 35s-quiet capture (needs a local Pusher-compatible websocket server); (3) `gate10-quality.yml` green on CI for the pushed branch. See `docs/implementation/gate-10-evidence.md`.
