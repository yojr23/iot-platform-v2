...
{
    /**
     * Build a frontend reset URL for the SPA instead of the Blade route.
     */
    protected function resetUrl($notifiable): string
    {
        $frontUrl = rtrim((string) config('app.front_url', config('app.url')), '/');

        return $frontUrl.'/reset-password?'.http_build_query([
            'token' => $this->token,
